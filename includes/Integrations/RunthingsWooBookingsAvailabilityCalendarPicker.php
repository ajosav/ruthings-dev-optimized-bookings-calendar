...
use WC_Product_Accommodation_Booking;
...
class RunthingsWooBookingsAvailabilityCalendarPicker
{
     ...
     */
    public function update_fully_booked_dates( $booked_data_array, $product, $min_date, $max_date) {
        ...
        $check_in_out_times = $this->get_check_in_and_out_times( $product, $min_date, $max_date );
...
//                    $check = date( 'F j, Y, g:i a', $check_time );
                    ...
                    $available_time_slots = $this->get_time_slots_available_for_resource($product, $resource_id_to_use, $min_date, $max_date);
                    $available_on_time  = $this->get_product_resource_available_blocks_on_time( $available_time_slots, $resource_id_to_use, $check_time );
    ...
    */
    private function get_check_in_and_out_times( $product, $min_date, $max_date ) {
        ...
        if ( version_compare( WC_BOOKINGS_VERSION, '1.15.0', '<' ) ) {
            // We can cache all existing bookings
            $existing_bookings = WC_Bookings_Controller::get_all_existing_bookings( $product, $min_date, $max_date );
        } else {
            $existing_bookings = WC_Booking_Data_Store::get_all_existing_bookings( $product, $min_date, $max_date );
        }
     ...
     *
     * @param array               $available_slots  product slots.
     * @param string              $time     Timestamp.
     *
     * @return int|mixed
     */
    private function get_product_resource_available_blocks_on_time( $available_slots, $time ) {
